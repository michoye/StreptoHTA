...
The second plot illustrates the distribution of total quality-adjusted life years (QALYs) per person. The strong peak near 1.5 QALYs suggests that most individuals achieve a stable quality of life, with little variation. The right-skewed distribution indicates that a few individuals may have slightly higher QALYs, potentially due to better health states or longer survival.

The third plot tracks the proportion of individuals in each health state over time. Initially, a large fraction transitions into the sick (S) state. Over time, many recover (S → H), but a subset remains chronically sick. The proportion of deceased individuals (D) gradually rises, reflecting normal mortality progression, while the healthy (H) population stabilizes.
...
-   **Strategy AB costs more (\$69 higher) but does not provide additional health benefits (same QALYs as SoC).**
...
Two functions were defined in the 'Functions_StreptoHTA`.R'` file. The first was the `decision_model()` function which takes in model parameters and outputs the Microsiulation results. The second is the `calculate_ce_out()` function which calls `decision_model()` and uses the resulting Micro-simulation results to compute the total costs, QALYs, returning a data frame of costs and QALYs.
...
-   Strategy AB is not cost-effective compared to Standard of Care.

-   The very small QALY gain does not justify the additional cost based on the calculated ICER.

-   If decision-makers prioritize cost savings, they should stick with Standard of Care rather than adopting Strategy AB.
...
df_params_owsa <- data.frame(pars = c("c_diagAB", "c_S"),
                             ...
                             max  = c(100, 1000)  # max parameter values
                             )
...
-   Strategy AB is highly sensitive to diagnostic costs (c_diagAB), meaning that if these costs rise, the strategy could become less cost-effective.

-   Both strategies are negatively affected by increases in treatment costs (c_S), but Strategy AB is more vulnerable.

-   If c_diagAB or c_S increase significantly, Standard of Care may be the preferred option in cost-effectiveness terms.

### 9.2.2 OWSA demonstrating other values
...
df_params_owsa <- data.frame(pars = c("p_HS", "u_S"),
                             min  = c(0.001,   0.10),  # min parameter values
...
-   The probability of getting sick has a strong negative impact on expected outcomes. If it increases, both strategies perform worse, and Strategy AB is more affected.

-   The utility of being sick positively impacts outcomes. Higher values improve both strategies, with Standard of Care maintaining a small advantage.

-   Standard of Care performs slightly better overall across both sensitivity analyses, suggesting it remains the more cost-effective choice under these parameter variations.

-   Thus, unless additional factors justify Strategy AB, the Standard of Care remains the preferred option under these conditions.
...
-   Standard of Care remains the dominant strategy under most cost assumptions.

-   Strategy AB is only cost-effective when the diagnostic cost for Standard of Care is high, while Strategy AB remains relatively cheap.

-   If the cost of Standard of Care continues rising beyond the threshold (\~30-35), switching to Strategy AB might become a viable alternative.
...
This plot suggests a trade-off between cost and effectiveness, where the decision depends on whether the additional cost for the higher-QALY strategy is justified under the willingness-to-pay threshold used in the analysis.
...
-   Strategy AB offers only a small increase in QALYs but at a substantially higher cost.

-   The decision to adopt Strategy AB depends on whether the additional cost is justifiable based on the healthcare budget and willingness-to-pay threshold.
